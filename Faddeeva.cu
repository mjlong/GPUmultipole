...
#  define C(a,b) CComplex<SETTYPE>(a,b)
...
static  SETTYPE my_copysign(SETTYPE x, SETTYPE y) { return y<0 ? -x : x; }
...
__device__  CComplex<SETTYPE> FADDEEVA(erfcx)(CComplex<SETTYPE> z, SETTYPE relerr)
{
  return FADDEEVA(w)(C(-imag(z), real(z)), relerr);
}

// compute the error function erf(x)
__device__  SETTYPE FADDEEVA_RE(erf)(SETTYPE x)
{
...
__device__  CComplex<SETTYPE> FADDEEVA(erf)(CComplex<SETTYPE> z, SETTYPE relerr)
{
  SETTYPE x = real(z), y = imag(z);
  ...
  SETTYPE mRe_z2 = (y - x) * (x + y); // Re(-z^2), being careful of overflow
  SETTYPE mIm_z2 = -2*x*y; // Im(-z^2)
 ...
 taylor:
  {
    CComplex<SETTYPE> mz2 = C(mRe_z2, mIm_z2); // -z^2
 ...
 taylor_erfi:
  {
    SETTYPE x2 = x*x, y2 = y*y;
    SETTYPE expy2 = exp(y2);
    return C
...
// erfi(z) = -i erf(iz)
__device__  CComplex<SETTYPE> FADDEEVA(erfi)(CComplex<SETTYPE> z, SETTYPE relerr)
{
  CComplex<SETTYPE> e = FADDEEVA(erf)(C(-imag(z),real(z)), relerr);
  return C(imag(e), -real(e));
}

// erfi(x) = -i erf(ix)
__device__  SETTYPE FADDEEVA_RE(erfi)(SETTYPE x)
{
...
// erfc(x) = 1 - erf(x)
__device__  SETTYPE FADDEEVA_RE(erfc)(SETTYPE x)
{
#if !defined(__cplusplus)
...
// erfc(z) = 1 - erf(z)
__device__  CComplex<SETTYPE> FADDEEVA(erfc)(CComplex<SETTYPE> z, SETTYPE relerr)
{
  SETTYPE x = real(z), y = imag(z);
...
__device__  SETTYPE FADDEEVA_RE(Dawson)(SETTYPE x)
{
  const SETTYPE spi2 = 0.8862269254527580136490837416705725913990; // sqrt(pi)/2
  return spi2 * FADDEEVA(w_im)(x);
}

// compute Dawson(z) = sqrt(pi)/2  *  exp(-z^2) * erfi(z)
__device__  CComplex<SETTYPE> FADDEEVA(Dawson)(CComplex<SETTYPE> z, SETTYPE relerr)
{
  const SETTYPE spi2 = 0.8862269254527580136490837416705725913990; // sqrt(pi)/2
  SETTYPE x = real(z), y = imag(z);
  ...
  if (y >= 0) {
    ...
    CComplex<SETTYPE> res = exp(mz2) - FADDEEVA(w)(z, relerr);
    ...
    CComplex<SETTYPE> res = FADDEEVA(w)(-z, relerr) - exp(mz2);
    return spi2 * C(-imag(res), real(res));
  }
 ...
 taylor_realaxis:
  {
    ...
    if (x2 > 1600) { // |x| > 40
      ...
      SETTYPE D = spi2 * FADDEEVA(w_im)(x);
      SETTYPE y2 = y*y;
      return C
        (D + y2 * (D + x - 2*D*x2)
...
__device__  static  SETTYPE sinc(SETTYPE x, SETTYPE sinx) { 
...
__device__  static  SETTYPE sinh_taylor(SETTYPE x) {
  return x * (1 + (x*x) * (0.1666666666666666666667
                           + 0.00833333333333333333333 * (x*x)));
}

__device__  static  SETTYPE sqr(SETTYPE x) { return x*x; }

// precomputed table of expa2n2[n-1] = exp(-a2*n*n)
// for SETTYPE-precision a2 = 0.26865... in FADDEEVA(w), below.
//static const SETTYPE expa2n2[] = {
static __device__  __constant__ SETTYPE expa2n2[] = {
...
__device__  CComplex<SETTYPE> FADDEEVA(w)(CComplex<SETTYPE> z, SETTYPE relerr)
{
  if (real(z) == 0.0)
  ...
  if (relerr <= DBL_EPSILON) {
    ...
    const SETTYPE pi = 3.14159265358979323846264338327950288419716939937510582;
    if (relerr > 0.1) relerr = 0.1; // not sensible to compute < 1 digit
    a = pi / sqrt(-log(relerr*0.5));
    c = (2/pi)*a;
    a2 = a*a;
  }
  const SETTYPE x = fabs(real(z));
  const SETTYPE y = imag(z), ya = fabs(y);

  CComplex<SETTYPE> ret = 0.; // return value

  SETTYPE sum1 = 0, sum2 = 0, sum3 = 0, sum4 = 0, sum5 = 0;
...
#define USE_CONTINUED_FRACTION 1 // 1 to use continued fraction for large |z|
  ...
  if (ya > 7 || (x > 6  // continued fraction is faster
    ...
    const SETTYPE ispi = 0.56418958354775628694807945156; // 1 / sqrt(pi)
    SETTYPE xs = y < 0 ? -real(z) : real(z); // compute for -z if y < 0
    if (x + ya > 4000) { // nu <= 2
      if (x + ya > 1e7) { // nu == 1, w(z) = i/sqrt(pi) / z
        // scale to avoid overflow
        if (x > ya) {
          SETTYPE yax = ya / xs; 
          SETTYPE denom = ispi / (xs + yax*ya);
          ...
          SETTYPE xya = xs / ya;
          SETTYPE denom = ispi / (xya*xs + ya);
          ret = C(denom, denom*xya);
        }
      }
      else { // nu == 2, w(z) = i/sqrt(pi) * z / (z*z - 0.5)
        SETTYPE dr = xs*xs - ya*ya - 0.5, di = 2*xs*ya;
        SETTYPE denom = ispi / (dr*dr + di*di);
        ret = C(denom * (xs*di-ya*dr), denom * (xs*dr+ya*di));
      }
    }
    else { // compute nu(z) estimate and do general continued fraction
      const SETTYPE c0=3.9, c1=11.398, c2=0.08254, c3=0.1421, c4=0.2023; // fit
      SETTYPE nu = floor(c0 + c1 / (c2*x + c3*ya + c4));
      SETTYPE wr = xs, wi = ya;
      for (nu = 0.5 * (nu - 1); nu > 0.4; nu -= 0.5) {
        // w <- z - nu/w:
        SETTYPE denom = nu / (wr*wr + wi*wi);
        wr = xs - wr * denom;
        wi = ya + wi * denom;
      }
      { // w(z) = i/sqrt(pi) / w:
        SETTYPE denom = ispi / (wr*wr + wi*wi);
  ...
  /* Note: The test that seems to be suggested in the paper is x <
     ...
     negligible in SETTYPE precision; I find that this happens for x >
    ...
    if (relerr == DBL_EPSILON) { // use precomputed exp(-a2*(n*n)) table
      if (x < 5e-4) { // compute sum4 and sum5 together as sum5-sum4
        const SETTYPE x2 = x*x;
        expx2 = 1 - x2 * (1 - 0.5*x2); // exp(-x*x) via Taylor
        // compute exp(2*a*x) and exp(-2*a*x) via Taylor, to SETTYPE precision
        const SETTYPE ax2 = 1.036642960860171859744*x; // 2*a*x
        const SETTYPE exp2ax =
          1 + ax2 * (1 + ax2 * (0.5 + 0.166666666666666666667*ax2));
        const SETTYPE expm2ax =
          1 - ax2 * (1 - ax2 * (0.5 - 0.166666666666666666667*ax2));
        for (int n = 1; 1; ++n) {
          const SETTYPE coef = expa2n2[n-1] * expx2 / (a2*(n*n) + y*y);
          prod2ax *= exp2ax;
          prodm2ax *= expm2ax;
          sum1 += coef;
          sum2 += coef * prodm2ax;
        ...
        expx2 = exp(-x*x);
        const SETTYPE exp2ax = exp((2*a)*x), expm2ax = 1 / exp2ax;
        for (int n = 1; 1; ++n) {
          const SETTYPE coef = expa2n2[n-1] * expx2 / (a2*(n*n) + y*y);
          prod2ax *= exp2ax;
          prodm2ax *= expm2ax;
      ...
      if (x < 5e-4) { // compute sum4 and sum5 together as sum5-sum4
        ...
        expx2 = 1 - x2 * (1 - 0.5*x2); // exp(-x*x) via Taylor
        for (int n = 1; 1; ++n) {
          const SETTYPE coef = exp(-a2*(n*n)) * expx2 / (a2*(n*n) + y*y);
          prod2ax *= exp2ax;
          prodm2ax *= expm2ax;
          sum1 += coef;
          sum2 += coef * prodm2ax;
    ...
    const SETTYPE expx2erfcxy = // avoid spurious overflow for large negative y
      y > -6 // for y < -6, erfcx(y) = 2*exp(y*y) to SETTYPE precision
      ? expx2*FADDEEVA_RE(erfcx)(y) : 2*exp(y*y-x*x);
    if (y > 5) { // imaginary terms cancel
      const SETTYPE sinxy = sin(x*y);
      ret = (expx2erfcxy - c*y*sum1) * cos(2*x*y)
        + (c*x*expx2) * sinxy * sinc(x*y, sinxy);
    }
    else {
      SETTYPE xs = real(z);
      const SETTYPE sinxy = sin(xs*y);
      const SETTYPE sin2xy = sin(2*xs*y), cos2xy = cos(2*xs*y);
      const SETTYPE coef1 = expx2erfcxy - c*y*sum1;
      const SETTYPE coef2 = c*xs*expx2;
    ...
    SETTYPE n0 = floor(x/a + 0.5); // sum in both directions, starting at n0
    SETTYPE dx = a*n0 - x;
    sum3 = exp(-dx*dx) / (a2*(n0*n0) + y*y);
    sum5 = a*n0 * sum3;
    SETTYPE exp1 = exp(4*a*dx), exp1dn = 1;
    int dn;
    for (dn = 1; n0 - dn > 0; ++dn) { // loop over n0-dn and n0+dn terms
      SETTYPE np = n0 + dn, nm = n0 - dn;
      SETTYPE tp = exp(-sqr(a*dn+dx));
      SETTYPE tm = tp * (exp1dn *= exp1); // trick to get tm from tp
      tp /= (a2*(np*np) + y*y);
      tm /= (a2*(nm*nm) + y*y);
      sum3 += tp + tm;
      sum5 += a * (np * tp + nm * tm);
    ...
    while (1) { // loop over n0+dn terms only (since n0-dn <= 0)
      SETTYPE np = n0 + dn++;
      SETTYPE tp = exp(-sqr(a*dn+dx)) / (a2*(np*np) + y*y);
      sum3 += tp;
      sum5 += a * np * tp;
...
__device__  static SETTYPE erfcx_y100(SETTYPE y100)
{
  switch ((int) y100) {
case 0: {
SETTYPE t = 2*y100 - 1;
...
__device__  SETTYPE FADDEEVA_RE(erfcx)(SETTYPE x)
{
  if (x >= 0) {
...
/* Given y100=100*y, where y = 1/(1+x) for x >= 0, compute w_im(x).
   ...
   compared to fitting the whole [0,1] interval with a single polynomial. */
__device__  static SETTYPE w_im_y100(SETTYPE y100, SETTYPE x) {
  switch ((int) y100) {
    case 0: {
      SETTYPE t = 2*y100 - 1;
...
__device__  SETTYPE FADDEEVA(w_im)(SETTYPE x)
{
  if (x >= 0) {
...
static SETTYPE relerr(SETTYPE a, SETTYPE b) {
