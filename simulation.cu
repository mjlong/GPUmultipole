...
__global__ void history(multipole U238, MemStruct Info, unsigned num_src, unsigned devstep){
  ...
  Info.thread_active[id] =  terminated*blockDim.x*gridDim.x + *Info.num_terminated_neutrons < num_src;
...
__global__ void remaining(multipole U238, double *devicearray, CComplex* cvector, unsigned* dwindows, MemStruct Info){
  //TODO:this is one scheme to match threads to 1D array, 
  //try others when real simulation structure becomes clear
  int id = blockDim.x * blockIdx.x + threadIdx.x;
  bool live = true;
  ...
  while(live){
    ...
    dwindows[id*MAXCNT+cnt%MAXCNT] = (int)sigA;
    localenergy = localenergy * rnd;
    live = (localenergy > 1.0);
    cnt = cnt + 1;
  }
