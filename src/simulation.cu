...
__global__ void history(MemStruct DeviceMem, unsigned num_src,int shift,unsigned banksize, unsigned unique){
  //float a = wdspp[0];
  //float dx = wdspp[1];
  //float mfp = wdspp[2];
  ...
  int nid = unique*id+(!unique)*int(curand_uniform_double(&localState)*banksize);
  nid = nid+num_src;
